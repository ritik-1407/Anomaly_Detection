...
class DeepAnTCNN(Module):
    def __init__(self, window, pred_window, in_channels, filter1_size, filter2_size,filter3_size,kernel_size, pool_size, stride):
        ...
        self.conv3 = Conv1d(in_channels=filter1_size, out_channels=filter2_size, kernel_size=kernel_size, stride=stride, padding = 0)
        ...
        # self.dim1 = int(((window - kernel_size + 1) / pool_size) * ((window - kernel_size + 1) / pool_size)) * filter3_size
