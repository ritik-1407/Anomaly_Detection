...
class Predictor():
    def __init__(self, window, pred_window, lr = 1e-5, batch_size = 45, in_channels=1, filter1_size = 128, filter2_size = 32,filter3_size = 8,
            kernel_size = 2, pool_size = 2, stride = 1):
        self.model = DeepAnTCNN(window, pred_window, in_channels, filter1_size, filter2_size,filter3_size, kernel_size, pool_size, stride)
        self.lr = lr
