...
dataframe = spark.sql("select * from synthetic_datasets.sine_platform")
...
df2 = dataframe
df2 = df2.toPandas()
train, test, test1 = df.iloc[:4000],df.iloc[4001:],df2.iloc[4001:]
print(len(train))
print(len(test))
...
scaler = StandardScaler() # mean centering - around zero,
...
plot_anomalies(test1[seq_size:],anomaly_df['testMAE'].values,0.984)
