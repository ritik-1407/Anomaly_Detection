...
dataframe = spark.sql("select * from synthetic_datasets.sine_mean")
...
df2 = dataframe
df2 = df2.toPandas()
train, test, ground_t = df.iloc[6001:],df.iloc[:6000],df2.iloc[:6000]
print(len(train))
print(len(test))
# print(len(ground_t))
# count_of_ones = (train["is_anomaly"][seq_size:]== 1).sum()
# print(count_of_ones)
...
plot_anomalies(ground_t[seq_size:],anomaly_df['testMAE'].values,1.32)
...
# True labels
y_true = ground_t["is_anomaly"][seq_size:]
